...
export async function GET() {
  const baseUrl = (process.env.NEXT_PUBLIC_SITE_URL || 'https://block-tales.wiki').replace(/\/$/, '')
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Block Tales Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} is a comprehensive Block Tales resource for Roblox RPG chapters, cards, bosses, items, badges, BUX locations, and update guides.`)
  lines.push('')
